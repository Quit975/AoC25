...
function IncrementScoreAt(score_table, x, y)
...
function RemoveRollAt(rolls_table, score_table, x, y)
    assert(type(rolls_table) == "table" and type(score_table) == "table" and type(x) == "number" and type(y) == "number");
    assert(x > 0 and x <= width and y > 0 and y <= height);
...
function SolveSecondPart()
    ...
    local roll_removed = true;
    while roll_removed do
        roll_removed = false;
        for y = 1, height do
            for x = 1, width do
                if rolls[y][x] == 1 then
                    if access_scores[y][x] < 4 then
                        solution = solution + 1;
                        RemoveRollAt(rolls, access_scores, x, y);
                        roll_removed = true;
                    end
                end
            end
